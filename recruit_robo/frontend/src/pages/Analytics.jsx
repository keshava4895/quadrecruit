...
function HBar({ label, value, max, color = '#93c5fd', sub, subColor }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      <div className="w-28 text-[10px] text-gray-500 text-right truncate flex-shrink-0">{label}</div>
      <div className="flex-1 flex items-center gap-2 min-w-0">
        <div className="flex-1 bg-gray-100 rounded-full h-3.5 overflow-hidden">
          <div className="h-full rounded-full transition-all duration-500"
            style={{ width: `${pct}%`, background: color }} />
        </div>
        <span className="text-[11px] font-semibold text-gray-700 w-6 text-right flex-shrink-0">{value}</span>
        {sub != null && (
...
function VBar({ label, value, max, color }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 flex-1">
      ...
      <span className="text-[9px] text-gray-500 text-center leading-tight capitalize w-full truncate px-1">
        {label}
      </span>
    </div>
  )
      ...
      {['Table', 'Chart'].map(v => (
        <button key={v} onClick={() => setView(v.toLowerCase())}
          className={`px-3 py-1 rounded-full text-[11px] font-medium transition-all ${
            view === v.toLowerCase() ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'
          }`}>
          {v}
        </button>
      ))}
...
function TimeFilter() {
  return (
    <div className="relative">
      <select className="text-[11px] border border-gray-200 rounded-md pl-2.5 pr-6 py-1 bg-white text-gray-600 appearance-none focus:outline-none cursor-pointer">
...
const TH   = 'px-4 py-2.5 text-left text-[11px] font-medium text-gray-500 bg-gray-50 border-b border-gray-100 whitespace-nowrap'
const TH_R = 'px-4 py-2.5 text-right text-[11px] font-medium text-gray-500 bg-gray-50 border-b border-gray-100 whitespace-nowrap border-l border-gray-100'
const TD   = 'px-4 py-2.5 text-xs text-gray-700 border-b border-gray-100'
const TD_R = 'px-4 py-2.5 text-xs text-right text-gray-700 border-b border-gray-100 border-l border-gray-100'
            ...
            <tbody>
              {avg && (
                <tr className="bg-gray-50/50 hover:bg-gray-50 transition-colors">
                  <td className={TD}><span className="font-medium text-gray-600">All Jobs (Avg.)</span></td>
              ...
              {(jobStats || []).map(j => (
                <tr key={j.jobId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className={`${TD_R} font-medium ${j.hired > 0 ? 'text-emerald-600' : 'text-gray-400'}`}>{j.hired}</td>
                  <td className={TD_R}>
                    <span className={j.conversion_rate >= 20 ? 'text-emerald-600 font-medium' : j.conversion_rate >= 10 ? 'text-amber-600 font-medium' : 'text-gray-400'}>
                      {j.conversion_rate}%
                    </span>
              ...
              {!jobStats?.length && (
                <tr><td colSpan={5} className="px-4 py-6 text-center text-xs text-gray-400">No jobs yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {view === 'table' ? (
        <div className="overflow-auto flex-1 max-h-64">
          <table className="w-full border-collapse text-xs">
            ...
            <tbody>
              <tr className="bg-gray-50/50 hover:bg-gray-50 transition-colors">
                <td className={TD}><span className="font-medium text-gray-600">All Candidates (Avg.)</span></td>
              ...
              {sourceEntries.map(([src, cnt]) => {
                ...
                return (
                  <tr key={src} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className={TD_R + ' text-gray-400'}>—</td>
                  </tr>
                )
              })}
            ...
            <>
              <div className="relative flex-shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-lg font-bold text-gray-900">{totalSrc}</span>
              ...
              <div className="flex flex-col gap-2.5">
                {donutSegs.map(d => (
                  <div key={d.name} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600 w-14">{d.name}</span>
...
function FunnelPanel({ jobs, selectedJob, setSelectedJob, funnel, funnelLoading }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 bg-gray-50/60 gap-2 flex-wrap">
        <ViewToggle view={view} setView={setView} />
        <div className="flex items-center gap-2">
          <div className="relative">
            <select value={selectedJob} onChange={e => setSelectedJob(e.target.value)}
              className="text-[11px] border border-gray-200 rounded-md pl-2.5 pr-6 py-1 bg-white text-gray-600 appearance-none focus:outline-none max-w-[160px] truncate cursor-pointer">
      ...
      {funnelLoading ? (
              ...
              <tbody>
                {funnelStages.map(s => (
                  <tr key={s.stage} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className={TD_R}><span className="font-semibold text-gray-800">{s.count}</span></td>
                ...
                {!funnelStages.length && (
                  <tr><td colSpan={5} className="px-4 py-6 text-center text-xs text-gray-400">Select a job to view funnel</td></tr>
                )}
              </tbody>
            </table>
          </div>
          {funnel && (
            <div className="flex gap-4 px-4 py-2 border-t border-gray-100 bg-gray-50/50 text-[11px] text-gray-500">
              <span>Total <strong className="text-gray-700">{funnel.total}</strong></span>
        ...
        <div className="px-4 pt-3 pb-2" style={{ minHeight: 200 }}>
          {funnelStages.length === 0 ? (
            <p className="text-xs text-gray-400 text-center py-6">Select a job to view funnel</p>
          ) : (
...
function HiringPanel({ overview }) {
  ...
  const rows = [
    { label: 'Total Candidates', value: total,    pct: null,           color: 'text-gray-900'    },
    ...
    { label: 'Active Jobs',      value: active,   pct: null,           color: 'text-gray-900'    },
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.label} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className={TD_R + ' text-gray-400'}>{r.pct ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="px-6 py-5 w-full">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400 ml-1">Recruitment insights</span>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          { label: 'Total Candidates', value: overview?.total_candidates ?? '—', color: 'text-gray-900'    },
          ...
          <div key={k.label} className="bg-white border border-gray-100 rounded-2xl shadow-sm px-4 py-3">
