...
function AvailabilityModal({ interviewer, onClose }) {
  ...
  useEffect(() => {
    ...
    availabilityApi.list(interviewer.interviewerId)
      ...
      .catch(e => setLoadErr(e?.response?.data?.detail || 'Failed to load slots'))
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-xs text-red-500 font-medium">{loadErr}</p>
              <button onClick={() => {
                ...
                availabilityApi.list(interviewer.interviewerId)
                  ...
                  .catch(e => setLoadErr(e?.response?.data?.detail || 'Failed to load slots'))
                  .finally(() => setLoading(false))
              }} className="mt-2 text-xs text-purple-600 hover:underline">Retry</button>
            </div>
          ) : slots.length === 0 ? (
            <div className="text-center py-8">
            ...
            <div className="space-y-1.5 mb-4">
              {slots.map(s => {
                const isPast = s.slot_date < today
                return (
                  <div key={s.slotId} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border ${
                    ...
                    s.is_booked    ? 'border-amber-100 bg-amber-50/40' :
                    'border-purple-100 bg-purple-50/40'
                  }`}>
                    <Clock className={`w-3.5 h-3.5 flex-shrink-0 ${isPast ? 'text-gray-300' : s.is_booked ? 'text-amber-400' : 'text-purple-400'}`} />
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400">{s.duration_mins} min
                        {isPast     && <span className="ml-1.5 text-gray-400 font-medium">· Past</span>}
                        {s.is_booked && !isPast && <span className="ml-1.5 text-amber-600 font-medium">· Booked</span>}
                      </p>
                    </div>
                    {!s.is_booked && !isPast && (
...
export default function Interviewers() {
  ...
  const totalAssigned  = filteredData.reduce((s, iv) => s + iv.total_assigned, 0)
  const totalSelected  = filteredData.reduce((s, iv) => s + iv.selected, 0)
  const totalRejected  = filteredData.reduce((s, iv) => s + iv.rejected, 0)
  const totalUpcoming  = filteredData.reduce((s, iv) => s + iv.upcoming_count, 0)
  ...
  return (
    <div className="px-6 py-5 w-full">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 bg-gray-50/60 gap-3">
          <span className="text-xs font-semibold text-gray-700">
            {period !== 'all'
              ? `${filteredData.length} of ${data.length} interviewer${data.length !== 1 ? 's' : ''}`
              : totalUpcoming > 0
          ...
          <div className="flex items-center gap-2">
            {period === 'custom' && (
              <>
                <input
                  ...
                  className="text-[11px] px-2 py-1.5 rounded-lg border border-purple-300 bg-purple-50 text-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-400"
                />
            ...
            <select
              ...
              className={`text-[11px] font-medium px-2.5 py-1.5 rounded-lg border transition-colors focus:outline-none focus:ring-2 focus:ring-purple-400 cursor-pointer ${
                period !== 'all'
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 font-medium">No interviews scheduled {period === 'week' ? 'this week' : period === 'month' ? 'this month' : 'this year'}</p>
            <button onClick={() => setPeriod('all')} className="mt-2 text-xs text-purple-600 hover:underline">Show all</button>
          </div>
        ) : (
